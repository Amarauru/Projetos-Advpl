...
Static Function fImporta()
    Local aArea      := GetArea()
    Local cArqLog    := "zImpoCSV_" + dToS(Date()) + "_" + StrTran(Time(), ':', '-') + ".log"
    Local nTotLinhas := 0
    Local cLinAtu    := ""
    Local nLinhaAtu  := 0
    Local aLinha     := {}
        ...
        If ! (oArquivo:EoF())
            ...
            While (oArquivo:HasLine())
                ...
                If !"codigo" $ Lower(cLinAtu)
                    ...
                    If SA2->(DbSeek(FWxFilial('SA2') + PADR(cCodForn,TAMSX3("A2_COD")[1]) + PADR(cLojForn,TAMSX3("A2_LOJA")[1])))
                        cLog += "+ Lin" + cValToChar(nLinhaAtu) + ", fornecedor [" + PADR(cCodForn,TAMSX3("A2_COD")[1]) + PADR(cLojForn,TAMSX3("A2_LOJA")[1]) + " - " + Upper(SA2->A2_NREDUZ) + "] " +;
                    ...
                    Else
                        cLog += "- Lin" + cValToChar(nLinhaAtu) + ", fornecedor e loja [" + PADR(cCodForn,TAMSX3("A2_COD")[1]) + PADR(cLojForn,TAMSX3("A2_LOJA")[1]) + "] não encontrados no Protheus;" + CRLF
                    EndIf
